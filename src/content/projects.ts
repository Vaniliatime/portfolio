...
export const projects: Project[] = [
  ...
  },
  {
    ...
    highlights: {
      en: [
        ...
        "A project asks for a database and gets one: its own database and its own role on the shared engine, a generated password, and the connection string already in its environment.",
        "A browser for those databases, with the tables and their sizes, the rows themselves and a SQL console that runs as the application's own role, so a query that would fail in production fails here too.",
        "A real terminal in the browser on any host the panel holds a key for: job control, colours and Ctrl+C all work, so the panel is not a reason to keep an SSH client open beside it.",
        "A container that has outgrown its limits gets more processor, memory or disk from the same screen, instead of a trip to the hypervisor.",
        "A database that was running before the panel existed can be adopted: it reads it and shows it, but has no delete button for it, because it did not create it.",
        "The machine as a whole on its own tab, not the containers on it: processor, load, memory, swap, network and pressure, from an hour to a month, with how full the disk pools are.",
      ...
      pl: [
        ...
        "Projekt prosi o bazę i ją dostaje: własną bazę i własną rolę na współdzielonym silniku, wygenerowane hasło i gotowy adres połączenia już w swoich zmiennych środowiskowych.",
        "Przeglądarka tych baz: tabele z rozmiarami, podgląd wierszy i konsola SQL działająca z uprawnieniami roli danej aplikacji, więc zapytanie, które nie zadziałałoby na produkcji, nie zadziała i tutaj.",
        "Prawdziwy terminal w przeglądarce na każdym hoście, do którego panel ma klucz: działa sterowanie zadaniami, kolory i Ctrl+C, więc panel nie jest powodem, żeby trzymać obok otwartego klienta SSH.",
        "Kontener, któremu zrobiło się ciasno, dostaje więcej procesora, pamięci albo dysku z tego samego ekranu, zamiast wycieczki do hipernadzorcy.",
        "Bazę, która działała przed panelem, da się zaadoptować: czyta ją i pokazuje, ale nie ma dla niej przycisku usuwania, bo jej nie tworzył.",
        "Cała maszyna na osobnej zakładce, a nie kontenery na niej: procesor, obciążenie, pamięć, swap, sieć i ciśnienie, w zakresie od godziny do miesiąca, razem z zajętością pul dyskowych.",
    ...
    technical: {
      en: [
        ...
        "Databases are provisioned rather than administered: one shared Postgres, a database and a least-privilege role per project, and the credentials injected as environment variables instead of being written down anywhere a person would read them. An adopted database is registered read-only, so the panel can never destroy something it did not create.",
        "The shell is a pty allocated through script and ssh -tt rather than a native module, which keeps the image free of a compiled dependency. The cost is visible and documented in the panel itself: resizing the window re-runs stty, which echoes once in the session.",
        "The server graphs come from Proxmox's own RRD rather than a metrics stack of my own. No second copy of the truth to drift out of agreement with the first, nothing to retain, and nothing to back up. They are inline SVG, drawn without a charting library.",
      ...
      pl: [
        ...
        "Bazy są zakładane, a nie administrowane: jeden współdzielony Postgres, osobna baza i rola o minimalnych uprawnieniach na projekt, a dane dostępowe wstrzykiwane jako zmienne środowiskowe, zamiast lądować gdziekolwiek, gdzie przeczytałby je człowiek. Zaadoptowana baza jest rejestrowana tylko do odczytu, więc panel nie zniszczy czegoś, czego nie stworzył.",
        "Powłoka to pty przydzielane przez script i ssh -tt, a nie natywny moduł, dzięki czemu obraz nie ciągnie za sobą kompilowanej zależności. Koszt jest widoczny i opisany w samym panelu: zmiana rozmiaru okna uruchamia stty ponownie, co raz odbija się echem w sesji.",
        "Wykresy serwera pochodzą z RRD samego Proxmoksa, a nie z własnego stosu metryk. Nie ma drugiej kopii prawdy, która mogłaby rozjechać się z pierwszą, nie ma czego przechowywać ani czego backupować. Rysowane jako inline SVG, bez biblioteki do wykresów.",
    ...
    gallerySections: [
      ...
      },
      {
        ...
        images: [
          "/images/prox-hub/databases.webp",
          "/images/prox-hub/database.webp",
          "/images/prox-hub/console.webp",
        ],
      },
      {
  ...
  {
    slug: "vanila-planner",
    title: "Vanila Planner",
    year: "2026",
    category: "app",
    status: "live",
    featured: true,
    tagline: {
      en: "A planner, a calendar and a kanban board, self-hosted, in one place.",
      pl: "Planer, kalendarz i tablica kanban, self-hostowane, w jednym miejscu.",
    },
    summary: {
      en: "Two ways of working that usually need two applications. Planner is the day: what is overdue, what is today, what comes back and when. Code is the same work as cards on an open canvas, where a column sits wherever you put it and decides the status of anything dropped into it. It runs on my own server, so the contents of my week are not somebody else's data.",
      pl: "Dwa tryby pracy, na które zwykle potrzeba dwóch aplikacji. Planner to dzień: co zaległe, co dzisiaj, co wraca i kiedy. Code to ta sama praca jako karty na otwartym płótnie, gdzie kolumna stoi tam, gdzie ją postawisz, i nadaje status temu, co do niej wrzucisz. Stoi na moim serwerze, więc zawartość mojego tygodnia nie jest cudzymi danymi.",
    },
    role: {
      en: "Solo: design, front end, back end, deployment.",
      pl: "Solo: projekt, front end, back end, wdrożenie.",
    ...
    highlights: {
      en: [
        "The day opens on what actually needs attention: overdue, today, and what is coming, with the day plan beside it.",
        "A month and a week view for anything with a time on it, and a task that repeats shows up on every day it falls on.",
        "Repeating tasks that survive real life: tick one occurrence, skip one, or move one, and the rest of the series carries on unchanged.",
        "The same work as a board when a list is the wrong shape: columns you place yourself, each one carrying the status it gives its cards.",
        "Categories and tags with colours of their own, which is what makes a crowded calendar readable at a glance.",
        "Reminders for the things that would otherwise be remembered at midnight.",
        "Everything exportable as readable JSON, screenshots included, and importable into another copy: an account can be moved, which is the point of hosting it yourself.",
        "The whole interface in Polish and English.",
      ],
      pl: [
        "Dzień otwiera się na tym, co naprawdę wymaga uwagi: zaległe, dzisiejsze i to, co nadchodzi, a obok plan dnia.",
        "Widok miesiąca i tygodnia dla wszystkiego, co ma godzinę, a zadanie cykliczne pojawia się w każdym dniu, na który wypada.",
        "Cykle, które znoszą zderzenie z życiem: odhacz jedno wystąpienie, pomiń jedno albo przesuń jedno, a reszta serii idzie dalej bez zmian.",
        "Ta sama praca jako tablica, gdy lista jest złym kształtem: kolumny ustawiasz sam, a każda nadaje status kartom, które w niej wylądują.",
        "Kategorie i tagi z własnymi kolorami, bo to one sprawiają, że zatłoczony kalendarz czyta się rzutem oka.",
        "Przypomnienia o rzeczach, które inaczej przypomniałyby się o północy.",
        "Wszystko do wyeksportowania jako czytelny JSON, razem ze zrzutami, i do zaimportowania w innej kopii: konto da się przenieść, i o to chodzi w trzymaniu tego u siebie.",
        "Cały interfejs po polsku i po angielsku.",
      ],
    },
    technical: {
      en: [
        "Next.js 16 on the App Router with React 19 and Server Actions, TypeScript, Tailwind 4, and Prisma 7 over PostgreSQL.",
        "Recurrence is never materialised. A repeating task is one RRULE (RFC 5545) expanded on the fly for whatever range is on screen, and only the deviations are stored: an occurrence ticked, skipped or moved. Daily forever therefore costs no extra rows at all.",
        "Authentication written by hand rather than pulled in: scrypt, sessions in the database with their tokens stored as SHA-256 hashes, rate limiting, email verification and password reset. The responses are deliberately identical whether or not an account exists, so the form cannot be used to find out who has one.",
        "Its own date, time and colour pickers, because the native ones cannot be themed for a dark interface. They take typed input and filter the list as you type, which the native ones do not.",
        "Both languages come from a typed dictionary, so a missing translation is a compile error rather than a string that ships in the wrong language.",
        "Deployed from a hand-written multi-stage Docker image behind Traefik and a Cloudflare Tunnel, with migrations applied when the container starts.",
        "Its database was not set up by hand: Prox Hub, the deploy panel further down this list, created the database and its role on the shared Postgres and put the connection string into this project's environment. The first thing that panel provisioned was this.",
      ],
      pl: [
        "Next.js 16 na App Routerze z React 19 i Server Actions, TypeScript, Tailwind 4 oraz Prisma 7 na PostgreSQL.",
        "Cykliczność nigdy nie jest materializowana. Zadanie powtarzalne to jedna reguła RRULE (RFC 5545) rozwijana w locie dla zakresu, który akurat jest na ekranie, a zapisywane są wyłącznie odstępstwa: wystąpienie odhaczone, pominięte albo przesunięte. Codziennie bez końca nie kosztuje więc ani jednego dodatkowego wiersza.",
        "Uwierzytelnianie napisane ręcznie, a nie dociągnięte: scrypt, sesje w bazie z tokenami trzymanymi jako skróty SHA-256, ograniczanie liczby prób, weryfikacja adresu i reset hasła. Odpowiedzi są celowo identyczne niezależnie od tego, czy konto istnieje, więc formularza nie da się użyć do sprawdzania, kto je ma.",
        "Własne wybieranie daty, godziny i koloru, bo natywnych nie da się ostylować pod ciemny interfejs. Przyjmują wpisywanie z klawiatury i filtrują listę w trakcie pisania, czego natywne nie robią.",
        "Oba języki pochodzą z typowanego słownika, więc brakujące tłumaczenie jest błędem kompilacji, a nie napisem, który wyjeżdża w złym języku.",
        "Wdrażane z ręcznie napisanego wieloetapowego obrazu Dockera za Traefikiem i tunelem Cloudflare, z migracjami wykonywanymi przy starcie kontenera.",
        "Baza nie była zakładana ręcznie: Prox Hub, panel wdrożeniowy z tej samej listy, utworzył bazę i jej rolę na współdzielonym Postgresie i wstawił adres połączenia do zmiennych tego projektu. To on był pierwszym, co ten panel założył.",
      ],
    },
    tech: [
      "Next.js 16",
    ...
    gallerySections: [
      {
        ...
        images: [
          ...
          "/images/planner/edit-task.webp",
          "/images/planner/categories.webp",
          "/images/planner/add-category.webp",
        ],
      },
      {
        label: { en: "Boards", pl: "Tablice" },
        images: [
      ...
      {
        label: { en: "Account", pl: "Konto" },
        images: [
          "/images/planner/login.webp",
          "/images/planner/registration.webp",
          "/images/planner/profile.webp",
          "/images/planner/account.webp",
          "/images/planner/settings.webp",
          "/images/planner/notifications.webp",
          "/images/planner/data.webp",
        ],
      },
    ],
    galleryAspect: "wide",
